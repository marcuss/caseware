...
The feature costs about $280 a month, or $1,090 at worst, against a ceiling I propose at $2,500. Inference is 87% of the expected $280, because the bill turns on one summary per version pair rather than one per file.
...
The service level objectives are measured and paged. For freshness, 99% of affected rows change within 60 seconds of the publish commit, per region, and a rendered summary exists for 95% of live pairs within 15 minutes. For projection health, `UNKNOWN` plus `unverified` stays under 1% after backfill, and drift under 0.1%, sampled by 200 random full loads a day. That sampling is the only alarm that sees silent drift, and a `NONE` row that is really eligible pages, because a firm may be skipping an evaluation it owed. For politeness, if engagement-open p99 rises 10% above baseline the verifier halves its cap, and spend stops at twice expected.

A materializer crash costs nothing to recover from, because SQS redelivers and the write is conditional. Verifier work is keyed on `(engagementId, trigger)`, so work already done is skipped on redelivery. An error that retrying cannot fix goes straight to the dead-letter queue, because five retries across 20,000 rows would burn 1,667 hours of the engagement team's capacity. Losing the projection means a restore and a replay of the 30-day event archive, not a 13,333-hour rebuild. Under any outage the row shows "checking" instead of "none".
